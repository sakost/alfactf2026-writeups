...
- `SLEEP()` в контексте константного выражения MySQL выкидывает на этапе оптимизации. Чтобы он действительно выполнился, нужно привязать подзапрос к реальному источнику строк — `(SELECT IF(..., SLEEP(t), 0) FROM real_table LIMIT 1)`.
