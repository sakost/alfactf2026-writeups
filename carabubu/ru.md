...
- Включён email-confirm (`CONF_ENABLE_REGCONFIRMATION`), но пользователь всё равно может войти.
...
- `mysql_real_escape_string` — не замена белому списку, когда небезопасный sink — это `ORDER BY`: метасимволы здесь не кавычки, а скобки и подзапросы.
- `SLEEP()` в константно-выражательном контексте оптимизатор выкидывает. Чтобы вынудить его выполнить, нужно привязать подзапрос к реальному источнику строк — `(SELECT IF(..., SLEEP(t), 0) FROM real_table LIMIT 1)`.
- Загрузка `.htaccess` превращает «белый список расширений для PHP» в фикцию: атакующий сам переопределяет, что считать PHP в подконтрольной ему директории. Любой загрузчик, не фиксирующий имя на сервере и не запрещающий `.htaccess`, — это один шаг до RCE на Apache.
