...
- `pidfd_getfd()` — правильный примитив для «одолжить fd у чужого процесса»: в отличие от `/proc/PID/fd/N`, который переоткрывает inode, этот вызов разделяет существующий open-file description, что критично для `/dev/pts/ptmx` (повторное открытие создало бы новую пару PTY).
