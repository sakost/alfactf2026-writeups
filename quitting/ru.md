...
Подключаемся по SSH к jumphost как пользователь `quitting` и получаем root-шелл. На jumphost уже есть активная сессия `admin`, который **с jumphost** подключён по SSH **к** `tracker-prod` — целевому серверу. Задача — перехватить эту сессию, попасть на tracker-prod и забрать флаг.
...
| Файл | Назначение |
|------|------------|
| `inject.sh` | Рабочий солвер — `pidfd_getfd()` дублирует мастер-PTY у sshd, в сессию `admin` пишется команда добавления нашего публичного ключа в `authorized_keys`, после чего заходим по SSH с этим ключом |
| `inject2.sh` | Неудачная предыдущая попытка — запись в `/proc/PID/fd/N` для `ptmx` создаёт *новую* пару PTY вместо доступа к существующей (оставлено как задокументированный тупик) |
