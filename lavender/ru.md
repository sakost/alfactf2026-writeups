...
В образе ext4-раздел со смещением 2048 внутри `CAFE.E01`. Читаем sleuthkit напрямую из E01 (macOS не монтирует ext4):
...
- v0.1 любого C2-фреймворка — это ещё не security-продукт: `tcLogin`, принимающий любой `username`, и `tcConnect`, безусловно поднимающий WebSocket без OTP, делают тимсервер тривиально подделываемым. И даже после OTP стоит искать асимметрию доверия (например, логин подписывает JWT, а другой code path его потребляет с другими проверками).
- Шаблоны типа `-DSERVICE_NAME='"%s"'` кажутся безопаснее, чем есть на самом деле. Как только `%s` может содержать одинарную кавычку, всё `'...'`-обрамление пробивается. Решение — `exec.Command(prog, args...)` (без шелла), а не «более умное» квотирование.
- Криминалистические образы CTF-машин — это чаще всего цепочка: артефакт → встроенный криптоматериал → внешний C2 → серверное хранилище ключей. Артефакт — *первая* зацепка, а не ответ; публичный ключ из шифровальщика — мост, который подтверждает, что нужный приватный ключ нашёлся.
