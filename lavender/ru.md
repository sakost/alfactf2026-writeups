...
- v0.1 любого C2-фреймворка — это ещё не зрелый продукт по части безопасности: `tcLogin`, принимающий любой `username`, и `tcConnect`, безусловно поднимающий WebSocket без OTP, делают тимсервер тривиально подделываемым. И даже после появления OTP стоит искать асимметрии доверия — когда логин подписывает JWT, а другая ветка кода потребляет этот JWT с уже другими проверками.
