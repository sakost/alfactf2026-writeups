...
Решающие скрипты: [`artifacts/`](artifacts/) — запускать примерно в таком порядке:

| Файл | Назначение |
|------|------------|
| `extract_roms.py` | Парсит синтезированный нетлист `peace.v` и выгружает все ROM `_M_xxxxx_` в `roms.json` (путь к `peace.v` — `argv[1]`; сам нетлист в репо не лежит) |
| `analyze_roms.py` | Sanity-проверка: какие ROM — перестановки и насколько их элементы отличаются от стандартного AES S-box |
| `check_sboxes.py` | Подтверждение, что `M_00000..M_00015` попарно XOR-сдвинуты от одной и той же 256-байтной перестановки |
| `check_ttables.py` | Идентификация каждого `M_00016..M_00031` как `S(i ⊕ a) ⊕ b` против стандартных AES T-таблиц `T0..T3` |
| `find_aes_keys.py` | Восстановление `(a_j, b_j)` побайтово для финальных S-box'ов (байты ключа раундов 9 и 10) |
| `aes_decrypt.py` | Чистый Python AES-128-ECB: подставляет восстановленный мастер-ключ и расшифровывает четыре отображённых блока |
